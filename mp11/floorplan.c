//This program uses binary trees (slicing trees) to do some basic calculations and implement them.
//we also use recursion in some functions for efficient results.

//MP Partners: Luis Aragon (laragon2) and Manan Mittal (manansm2);
...
int is_internal_node(node_t* ptr) {
  ...
  //if it has children then its an internal node. Parents don't matter since root node is considered internal.
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  //if a and b are the same node then b resides in subtree rooted at a.
  //else as long as a is an internal node then keep checking if we ever get to
  //the b node as we traverse through all of a's children. If we hit b then it is a subtree.
  if(a==b)
  {
    return 1;
  }
  else if(is_internal_node(a))
  {
...
void rotate(node_t* ptr) {
  ...
  //instructions say rotate is just swapping the height and width. Don't completely understand what I am rotating here.
  int x = ptr->module->h;
...
void recut(node_t* ptr) {
  ...
  //if the current cutline is vertical then make the cutline horizontal, vice versa.
  if(ptr->cutline==V)
  {
    ptr->cutline=H;
  }
...
void swap_module(node_t* a, node_t* b) {
  ...
  //simple swap function to swap the modules of two nodes.
  module_t *x = a->module;
  a->module = b->module;
  b->module = x;
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
  //set the temp vars to hold values
  node_t* x = a;
  node_t* bp = b->parent;
  node_t* xparent = a->parent;

  //swap childs of a and b's parents and swap parents of a and b.
  //determine if a is on left or right side of its parent first, then swap.
  if(a->parent->left==a)
  {
    a->parent->left = b;
    b->parent=xparent;
...
// Perform the postfix traversal on the slicing tree and store the corresponding polish (lol what?) expression
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  //traverse through left and right subtrees recursively and at the end when
  //we hit base case return the values at each point in the recursive instance.
...
int get_total_resource(node_t* ptr)
{
  ...
  int i;
  int total = 0;
  //go through array of modules and add up the value of the resources.
  for(i = 0; i<num_modules;i++)
  {
    total = total + modules[i].resource;
  }
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  //if we are at the base (end) case where we have hit the number of modules we need
  //and make the tree then we add the final nodes with no more children and return the leftmost bottom node.
  if(n+1 == num_modules)
  {
    node_t* lp = (node_t*)malloc(sizeof(node_t));
    node_t* rp = (node_t*)malloc(sizeof(node_t));
    (*par).left = lp;
    par->right = rp;


    lp->module=&modules[n];
    lp->cutline=UNDEFINED_CUTLINE;
    lp->parent = par;
    lp-> left = NULL;
    lp-> right = NULL;

    rp->module=&modules[n-1];
    rp->cutline=UNDEFINED_CUTLINE;
    rp->parent = par;
    rp-> left = NULL;
    rp-> right = NULL;
    return lp;
  ...
  //if not at base case, then keep building a left skewed tree.
  node_t* lp = (node_t*)malloc(sizeof(node_t));
  lp->module=NULL;
  lp->cutline=V;
  lp->parent = par;

  //if it isn't the root node then also also add a right side node.
  if(par != NULL)
  {
    node_t* rp = (node_t*)malloc(sizeof(node_t));
